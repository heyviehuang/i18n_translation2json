...
const TEMPLATES = [
    ({ word, zh, showZh, q, total, remain, seed }) => [
        ...
        `  < span class="kw">const</> <span class="var">res</span> <span class="op">=</span> <span class="kw">await</span> <span class="var">jsonp</span>({ <span class="prop">action</span><span class="op">:</span> <span class="str">"nextBatch"</span>, <span class="prop">count</span><span class="op">:</span> <span class="var">ROUND_SIZE</span>})`,
        ...
        `    < span class="var">speak</>(<span class="var">batch</span>[<span class="var">ix</span>].< span class="prop" > word</ >)`,
...
function copyText(txt) {
    if (!txt) return;
    if (navigator.clipboard?.writeText) navigator.clipboard.writeText(txt).then(() => toast('copied'));
    else {
        const ta = document.createElement('textarea'); ta.value = txt; document.body.appendChild(ta);
        ta.select(); document.execCommand('copy'); ta.remove(); toast('copied');
    }
}
function copyCurrentEN() { if (ix >= 0 && ix < batch.length) copyText(batch[ix].word); }
function copyCurrentZH() { if (ix >= 0 && ix < batch.length) copyText(batch[ix].zh || ''); }

// 綁定按鈕（記得這兩個元素存在）
document.getElementById('copyEn').addEventListener('click', (e) => { e.stopPropagation(); copyCurrentEN(); });
document.getElementById('copyZh').addEventListener('click', (e) => { e.stopPropagation(); copyCurrentZH(); });
